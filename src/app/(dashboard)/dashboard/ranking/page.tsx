...
import { Trophy, Medal, Star, Dumbbell, TrendingUp, Zap, Clock, Users, UsersRound } from 'lucide-react'
...
type Props = { searchParams: Promise<{ personal?: string }> }

export default async function RankingDashboardPage({ searchParams }: Props) {
  ...
  // Papel efetivo + tipo de tenant — espelha getCtx() (src/lib/supabase/context.ts).
  // Numa academia, o ranking "global" (entre TODOS os tenants da plataforma) não
  // interessa: a academia só quer ver os próprios alunos, com a possibilidade de
  // filtrar por personal — isso é o que permite premiar os melhores alunos e
  // identificar quais personais tiveram os alunos com melhor desempenho.
  const { data: tenant } = tenantId
    ? await supabase.from('tenants').select('tenant_type').eq('id', tenantId).single()
    : { data: null }
  ...
  // Histórico de campeões (últimos 6 meses) — segue sendo o "Hall da Fama"
  // global da plataforma, independente de academia/autônomo.
  ...
  // ── Ramo Academia — ranking próprio + filtro/leaderboard por personal ──────
  if (isAcademia) {
    type AcademiaStudentRow = { id: string; full_name: string; avatar_url: string | null; assigned_personal_id: string | null } | null

    const { data: academiaPoints } = await supabase
      .from('monthly_points')
      .select(`
        student_id, total_points, workouts_completed, exercises_completed,
        load_increases, active_minutes,
        students ( id, full_name, avatar_url, assigned_personal_id )
      `)
      .eq('tenant_id', tenantId!)
      .eq('month', month)
      .eq('year', year)
    ...
    type AcademiaRow = typeof academiaPoints extends (infer T)[] | null ? T : never
    const academiaRanking = (academiaPoints ?? []).map((row: AcademiaRow, idx) => ({ ...row, rank_position: idx + 1 }))

    // Personais ativos da academia (para o filtro e para os nomes no leaderboard)
    ...
    type TeamRow = typeof teamRows extends (infer T)[] | null ? T : never
    const personalOptions = (teamRows ?? []).map((t: TeamRow) => ({
      id: t.id,
      role: t.role,
      name: joinOne<{ full_name: string }>(t.profiles)?.full_name ?? 'Sem nome',
    }))
    const personalNameById = new Map(personalOptions.map((p) => [p.id, p.name]))

    const { personal: filterPersonalId } = await searchParams

    const filteredRanking = filterPersonalId
      ? academiaRanking.filter((r) => (r.students as unknown as AcademiaStudentRow)?.assigned_personal_id === filterPersonalId)
      : academiaRanking

    // Agregado por personal — total de pontos dos alunos de cada um, pra
    // identificar quem teve os alunos com melhor desempenho no mês.
    const aggByPersonal = new Map<string, { totalPoints: number; studentCount: number; bestPos: number }>()
    academiaRanking.forEach((r) => {
      const st = r.students as unknown as AcademiaStudentRow
      const pid = st?.assigned_personal_id
      if (!pid) return
      const cur = aggByPersonal.get(pid) ?? { totalPoints: 0, studentCount: 0, bestPos: Infinity }
      cur.totalPoints += r.total_points
      cur.studentCount += 1
      cur.bestPos = Math.min(cur.bestPos, r.rank_position)
      aggByPersonal.set(pid, cur)
    })
    const personalLeaderboard = Array.from(aggByPersonal.entries())
      .map(([pid, agg]) => ({ personalId: pid, name: personalNameById.get(pid) ?? 'Sem personal', ...agg }))
      .sort((a, b) => b.totalPoints - a.totalPoints)

    const totalParticipants = academiaRanking.length
    const bestStudent        = academiaRanking[0] ?? null
    const bestStudentRow     = bestStudent ? (bestStudent.students as unknown as AcademiaStudentRow) : null

    const pillClass = (active: boolean) =>
      `text-xs font-semibold px-3 py-1.5 rounded-full border transition-colors ${
        active
          ? 'text-brand-lime bg-brand-lime/10 border-brand-lime/30'
          : 'text-text-secondary bg-surface border-surface-border hover:border-brand-lime/20'
      }`
    ...
    return (
      <div className="p-6 md:p-8 space-y-6">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-text-secondary text-sm mt-1">
              {MONTH_NAMES[month]} {year} · Ranking interno da academia
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Alunos participando', value: totalParticipants, icon: Users,      color: 'text-blue-400   bg-blue-400/10   border-blue-400/20'   },
            { label: 'Personal na equipe',   value: personalOptions.length, icon: UsersRound, color: 'text-brand-lime bg-brand-lime/10 border-brand-lime/20' },
            { label: 'Líder do mês',         value: bestStudentRow?.full_name ?? '—', icon: Trophy, color: 'text-yellow-400 bg-yellow-400/10 border-yellow-400/20' },
        ...
        {personalOptions.length > 0 && (
          <div className="flex flex-wrap gap-2">
            <Link href="/dashboard/ranking" className={pillClass(!filterPersonalId)}>
              Todos os personais
            </Link>
            {personalOptions.map((p) => (
              <Link key={p.id} href={`/dashboard/ranking?personal=${p.id}`} className={pillClass(filterPersonalId === p.id)}>
                {p.name}
              </Link>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          {/* Ranking da academia (respeitando o filtro por personal) */}
          <div className="bg-surface border border-surface-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3.5 border-b border-surface-border">
              ...
              <p className="text-xs font-semibold text-text-secondary uppercase tracking-widest">
                {filterPersonalId ? `Alunos de ${personalNameById.get(filterPersonalId) ?? 'personal'}` : 'Ranking da Academia'}
              </p>
            </div>
            ...
            {filteredRanking.length === 0 ? (
              ...
              <div className="divide-y divide-surface-border max-h-[480px] overflow-y-auto">
                {filteredRanking.map((r) => {
                  ...
                  const personalName = st?.assigned_personal_id ? personalNameById.get(st.assigned_personal_id) : null
                  return (
          ...
          {/* Ranking por personal — para identificar quem tem os melhores alunos */}
          <div className="bg-surface border border-surface-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3.5 border-b border-surface-border">
              <UsersRound size={14} className="text-text-secondary" />
              <p className="text-xs font-semibold text-text-secondary uppercase tracking-widest">
            ...
            {personalLeaderboard.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-text-secondary text-sm gap-3">
                ...
                <p>Nenhum personal com alunos pontuando ainda.</p>
              </div>
            ) : (
              <div className="divide-y divide-surface-border max-h-[480px] overflow-y-auto">
                {personalLeaderboard.map((p, idx) => (
                  <Link
                    key={p.personalId}
                    href={`/dashboard/ranking?personal=${p.personalId}`}
                    className="flex items-center gap-3 px-5 py-3.5 hover:bg-surface-border/10"
                  >
                    <MedalIcon pos={idx + 1} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-body font-semibold text-text-primary truncate">{p.name}</p>
                      <p className="text-xs text-text-secondary">
                        {p.studentCount} aluno{p.studentCount !== 1 ? 's' : ''} · melhor posição #{p.bestPos}
                      </p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className="font-display font-bold text-brand-lime text-lg">{p.totalPoints.toLocaleString('pt-BR')}</p>
                      <p className="text-[10px] text-text-secondary">pts (soma dos alunos)</p>
                    </div>
                  </Link>
                ))}
              </div>
  ...
  // ── Ramo Autônomo — comportamento original, com ranking global da plataforma ──
