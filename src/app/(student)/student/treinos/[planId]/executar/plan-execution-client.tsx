...
export function PlanExecutionClient({ plan }: { plan: WorkoutPlanWithRoutines }) {
  const router = useRouter()

  // Achata todos os exercícios de todas as rotinas em uma lista única
  const flatItems: FlatItem[] = plan.workout_routines.flatMap((routine) =>
    routine.workout_items.map((item, i) => ({
      item,
      routineName: routine.name,
      isFirstInRoutine: i === 0,
    }))
  )
  ...
  useEffect(() => {
    if (restActive && restRemaining > 0) {
      restRef.current = setInterval(() => {
        setRestRemaining((r) => {
          if (r <= 1) { setRestActive(false); return 0 }
          return r - 1
        })
      }, 1000)
    } else {
  ...
  const ex                = current?.item.exercises
  const st                = exState[current?.item.id ?? ''] ?? { loadUsed:'', setsDone:0, repsDone:'', feedback:'', completed:false }
  const currentComboLetter = current?.item.combo_group_id ? comboLetters[current.item.id] : null
      ...
      <>
        <div className="p-5 space-y-6">
          ...
          <div className="bg-surface border border-surface-border rounded-2xl p-5 space-y-2">
            <p className="text-xs text-text-secondary uppercase tracking-widest">{plan.name}</p>
            <p className="text-sm text-text-secondary">
              {plan.workout_routines.length} grupo{plan.workout_routines.length !== 1 ? 's' : ''} · {total} exercício{total !== 1 ? 's' : ''}
            </p>
          </div>

          {/* Exercícios agrupados por rotina */}
          {plan.workout_routines.map((routine) => (
            <div key={routine.id} className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs font-bold text-text-primary uppercase tracking-widest">{routine.name}</p>
                <span className="text-xs text-text-secondary">— {routine.workout_items.length} exercício{routine.workout_items.length !== 1 ? 's' : ''}</span>
              </div>

              <div className="space-y-2">
                {routine.workout_items.map((item) => {
    ...
    <div className="flex flex-col h-[calc(100dvh-64px)] overflow-hidden">
      ...
      <div className="px-5 py-3 border-b border-surface-border bg-background flex items-center gap-3 flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[11px] text-text-secondary truncate">{plan.name}</p>
        ...
        <div className="bg-surface border border-surface-border rounded-2xl p-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <span className="text-sm font-semibold text-text-primary bg-background border border-surface-border rounded-lg px-3 py-1.5">
              {countLine(current.item)}
            </span>
            {current.item.rest_seconds != null && current.item.rest_seconds > 0 && (
              <span className="flex items-center gap-1 text-xs text-text-secondary bg-background border border-surface-border rounded-lg px-3 py-1.5">
                <Clock size={11} /> {current.item.rest_seconds}s descanso
              </span>
            )}
        ...
        <div className="bg-surface border border-surface-border rounded-2xl p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => updateEx(current.item.id, { setsDone: Math.max(0, st.setsDone - 1) })}
                ...
                >−</button>
                <span className="flex-1 text-center font-display font-bold text-text-primary text-lg">{st.setsDone}</span>
                <button
                  onClick={() => updateEx(current.item.id, { setsDone: st.setsDone + 1 })}
                  className="w-8 h-8 rounded-lg border border-surface-border text-text-secondary hover:text-text-primary hover:bg-surface-border/30 transition-colors flex items-center justify-center text-lg font-light"
                >+</button>
              </div>
            </div>
            <div className="space-y-1">
              ...
              <input
                type="text"
                value={st.loadUsed}
                onChange={(e) => updateEx(current.item.id, { loadUsed: e.target.value })}
                placeholder="ex: 20kg"
          ...
          <div className="space-y-1">
            ...
            <textarea
              value={st.feedback}
              onChange={(e) => updateEx(current.item.id, { feedback: e.target.value })}
              rows={2}
          ...
          <div className="flex gap-2">
            <button
              onClick={() => markComplete(current.item.id)}
            ...
            {current.item.rest_seconds != null && current.item.rest_seconds > 0 && (
              <button
                onClick={() => restActive ? stopRest() : startRest(current.item.rest_seconds!)}
