...
import { NewAssessmentForm, AssessmentHistoryCard } from './assessment-form'
...
export default async function AvaliacaoPage() {
  ...
  const { data: assessments } = await supabase
    ...
    .select('id, assessed_at, weight, height, body_fat, arm, chest, waist, hip, thigh, notes')
    .eq('student_id', student.id)
    .order('assessed_at', { ascending: false })

  const list = assessments ?? []

  // Calcula variação de peso entre a última e penúltima avaliação
    ...
    <div className="p-5 md:p-8 space-y-6 max-w-2xl">
      ...
      {list.length > 0 && (
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-surface border border-surface-border rounded-2xl p-4">
            <p className="text-xs text-text-secondary uppercase tracking-widest mb-1">Variação de Peso</p>
            {weightDelta !== null ? (
