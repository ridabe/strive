...
import { assignMealPlanToStudents, removeMealPlanAssignment } from '@/app/actions/meal-plans'
...
export function MealPlanAssignPanel({ planId, allStudents, assignments }: Props) {
  ...
  function handleAssign() {
    ...
    startTransition(async () => {
      const result = await assignMealPlanToStudents(planId, selected)
  ...
  function handleRemove(studentId: string) {
    startTransition(async () => { await removeMealPlanAssignment(planId, studentId) })
  }
      ...
      {open && (
        <div className="border-t border-surface-border p-5 space-y-4">
          ...
          {unassigned.length > 0 && (
            <div className="space-y-2">
                ...
                {unassigned.map((s) => (
                  ...
                  >
                    <div className={`w-4 h-4 rounded border flex-shrink-0 transition-colors ${
                      selected.includes(s.id) ? 'bg-brand-lime border-brand-lime' : 'border-surface-border'
                    }`} />
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-1.5 py-2.5 bg-brand-lime text-background font-semibold text-sm rounded-xl hover:bg-brand-lime/90 transition-colors disabled:opacity-50"
              >
                <UserPlus size={14} />
          ...
          {unassigned.length === 0 && assignments.length === 0 && (
            <p className="text-sm text-text-secondary text-center py-4">Nenhum aluno ativo no tenant.</p>
          )}
          {unassigned.length === 0 && assignments.length > 0 && (
            <p className="text-xs text-text-secondary text-center">Todos os alunos já têm este plano atribuído.</p>
          )}
        </div>
      )}
    </div>
