...
(async () => {
  const usage = await sb.from("ai_usage_events").select("id, feature_type, client_platform, status, created_at, input_tokens, output_tokens, latency_ms, metadata, error_message").eq("feature_type", "suggest_load").order("created_at", { ascending: false }).limit(5);
  const messages = await sb.from("ai_messages").select("id, role, metadata, created_at, conversation_id").order("created_at", { ascending: false }).limit(10);
