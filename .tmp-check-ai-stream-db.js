...
(async () => {
  const usage = await sb.from("ai_usage_events").select("id, feature_type, client_platform, status, created_at, input_tokens, output_tokens, metadata, error_message").in("feature_type", ["suggest_load", "analyze_progress"]).order("created_at", { ascending: false }).limit(10);
  const messages = await sb.from("ai_messages").select("id, role, metadata, created_at, conversation_id").order("created_at", { ascending: false }).limit(12);
